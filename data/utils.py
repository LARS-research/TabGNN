...
def build_db_info(db_name, db_info, test_dp_query, train_dp_query):
    db_info_path = os.path.join(project_root, 'data', db_name, '{}.db_info_fz.json'.format(db_name))
...
def create_datapoints_with_xargs(db_name, datapoint_ids, base_query, target_dir, n_jobs):
    ...
    WARNING: if the tqdm process bar isn't increasing, there's probably a silent error in the called process.
        Copy and run the printed command in the terminal to debug.
    """
    datapoint_ids = np.array(datapoint_ids)
    datapoint_ids = datapoint_ids[datapoint_ids%1000==0]
    temp_file = os.path.join(target_dir, 'jobs.txt')
    with open(temp_file, 'w') as f:
        f.writelines('\n'.join([str(dp_id) for dp_id in datapoint_ids]))
    cmd = "cat {} | xargs -I DPID --max-procs={} -n 1 {} -m {} {} DPID '{}' {}".format(temp_file,
...
def get_db_container(db_name):
    ...
    source_dir = os.path.join(data_root.replace('home','data'), 'raw_data', db_name)
    ...
    container = client.containers.run('rdb-neo4j',
                                      ...
                                      environment=["NEO4J_dbms_active__database={}.graph.db".format(db_name), 
                                                   "NEO4J_dbms_memory_heap_max__size=10240M",
    ...
    time.sleep(15)  # Let the DB initialize
    return container.id
...
def train_val_split(dp_ids, dataset_name):
    data=pd.read_csv(os.path.join(data_root,'/raw_data/jd_data/train_embedding_100.csv'))
    data = np.array([list(data['main_id']),list(data['time'])]).T
    data = sorted(data.tolist(), key=lambda x:x[1])
    ids_sort = [each[0] for each in data]
    train_ids = np.array(ids_sort[0: int(0.95*len(ids_sort))])
    test_ids = np.array(ids_sort[int(0.95*len(ids_sort)): ])
